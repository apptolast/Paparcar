 ...
 *
 * Enqueued by [WorkManagerParkingSyncScheduler.enqueueUpdateParkingSessionAddressAndPlace] when
 * [UserParkingRepositoryImpl.updateParkingSessionAddressAndPlace] finishes its Room update. The
 * repository call is Room-only; this worker handles the Firestore side so
 ...
 *
 * Input data: [KEY_SESSION_ID], plus individual address/placeInfo fields.
 * Constraints: NETWORK_CONNECTED. Backoff: exponential 30 s base.
 */
class UpdateParkingSessionAddressAndPlaceWorker(
    context: Context,
    ...
    override suspend fun doWork(): Result {
        ...
        PaparcarLogger.d(TAG, "▶ UpdateParkingSessionAddressAndPlaceWorker.doWork session=$sessionId attempt=$runAttemptCount")

        return runCatching {
            userProfileDataSource.updateParkingSessionAddressAndPlace(userId, sessionId, address, placeInfo)
        }.fold(
            onSuccess = {
                PaparcarLogger.d(TAG, "■ UpdateParkingSessionAddressAndPlaceWorker SUCCESS session=$sessionId")
                Result.success()
            },
            onFailure = { e ->
                if (runAttemptCount < MAX_RETRY_ATTEMPTS) {
                    ...
                    // Known race: SaveNewParkingSessionWorker may not have created the Firestore document
                    // yet when this worker fires. Log as warning — not a Crashlytics non-fatal.
                    PaparcarLogger.w(TAG, "✗ giving up session=$sessionId after $MAX_RETRY_ATTEMPTS retries: ${e.message}")
                    Result.failure()
                }
            },
        )
    ...
    companion object {
        const val TAG = "PARKDIAG/UpdateParkingSessionAddressAndPlaceWorker"
        ...
        fun buildRequest(
            ...
            OneTimeWorkRequestBuilder<UpdateParkingSessionAddressAndPlaceWorker>()
                .setInputData(
                    workDataOf(
