...
// ─── Pap palette (mirrored from Color.kt — no Compose dependency here) ────────
private const val COLOR_PAP_GREEN       = 0xFF25F48C.toInt()
private const val COLOR_PAP_FOREST_DARK = 0xFF0D3D2E.toInt()
private const val COLOR_PAP_FOREST      = 0xFF0D1C14.toInt()
private const val COLOR_AMBER_ACCENT    = 0xFFF4A825.toInt()
private const val COLOR_AMBER_MUTED     = 0xFF3D2A10.toInt()

// ─────────────────────────────────────────────────────────────────────────────
// My parked car — dark-green pin with a DirectionsCar icon
// Size: 48 × 56 dp
// ─────────────────────────────────────────────────────────────────────────────
 ...
 *
 * Shape: dark-green circle (PapForestDark) with a DirectionsCar icon tinted PapGreen.
 * Border: 2.5 dp PapGreen stroke.
 */
fun Context.createMyCarMarkerIcon(): BitmapDescriptor {
    ...
    val carDrawable = ContextCompat.getDrawable(this, R.drawable.ic_pap_car)
    if (carDrawable != null) {
        DrawableCompat.setTint(DrawableCompat.wrap(carDrawable), COLOR_PAP_GREEN)
        val iconSize = (r * 1.1f).toInt()   // ≈ 55% of diameter
        val iconLeft = (cx - iconSize / 2f).toInt()
        val iconTop  = (cy - iconSize / 2f).toInt()
        carDrawable.setBounds(iconLeft, iconTop, iconLeft + iconSize, iconTop + iconSize)
        carDrawable.draw(canvas)
    }

    // 4 ─ Circle border (PapGreen glow ring, drawn last)
 ...
 *
 * Shape: bright-green circle (PapGreen) with a bold dark "P" and
 * a pointed tail below.
 * Border: 1.5 dp PapForest stroke around the circle.
 */
 ...
 *
 * Shape: amber circle (PapAmber) with a bold dark "P" and a tail.
 */
