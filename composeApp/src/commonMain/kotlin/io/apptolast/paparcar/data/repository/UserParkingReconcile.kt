 ...
 * remote wins, but the on-device-only detection provenance ([tripMaxSpeedMps], [armEvidence] — never
 * written to Firestore, same precedent as vehicle `licensePlate`) is carried over from the local
 * row so a remote-wins merge doesn't blank it. Once the server catches up
