...
Reap latency ≤ 15 min (periodic) vs the ~2 h observed. Silent by design (respects OEM-KILL-001 "earn the ask": no nudge when nothing was lost; the `shouldNudgeForStalePending` real-trip nudge is unchanged and independent). **F2 (teardown lean of the false trigger — release FGS + location before honest-close I/O, revisit the 4-min `no_movement` window)** is an open audit in `docs/backlog/det-fgs-reaper-001.md`, not in this change. ⏳ Pending: build + device field-test (force a walking `false_enter`, confirm the ghost clears within 15 min without a nudge).
