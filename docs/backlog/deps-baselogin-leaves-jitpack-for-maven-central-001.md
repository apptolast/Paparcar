...
· ⏳ queda que CI verifique el linkeo de iOS en el Mac (no es verificable en Windows, ver abajo)
...
## Rebase 31-08-2026 sobre master `c5bfd274`

La rama local se perdió (se cerró Android Studio con las sesiones abiertas) y la copia de `origin`
la borró el merge del PR en GitHub; se recuperó desde el SHA colgante `a0873c8a` y se rebaseó sobre
`master` local.

- **Sin conflictos.** 3 commits replicados: `060c568b` (migración) · `729ce3fa` (iosMain) ·
  `0aaf2aa6` (guardrail de wildcards).
- Git descartó por patch-id `81bacb11` · `02b555df` · `e5a32746` — los tres tickets de Historial de
  Vehículos que ya habían entrado en master por squash (`b0d40353` · `24009eb3` · `31adead8`). La
  rama los llevaba encima porque se trabajó sobre ella; no eran suyos.
- Verificado en el worktree `../Paparcar-baselogin`: `:shared:testDebugUnitTest` ✅ ·
  `:app:compileMockDebugKotlin` + `:app:compileProdDebugKotlin` ✅. iOS sigue sin verificarse en
  Windows (ver arriba).
- ⚠️ `origin/master` está en `7a4f90e0`, un squash de esta rama hecho **desde GitHub** sobre una base
  del 29-08 (`3d6e24cc`): 387 ficheros. Diverge de `master` local, que va 57 commits por delante de
  esa base. **No hacer `pull` de `origin/master`.**
