...
### Diálogo de reintentar (relacionado, decisión tomada)
Hoy `SplashViewModel.bootstrap:169` aborta a `BootstrapFailure.Offline` en **cualquier** cold-start offline. Gatearlo a **offline + sin cache**: si hay perfil+vehículos en Room → saltar sync remoto, resolver ruta desde Room y entrar (banner informa); sin cache → diálogo retry. Commit propio (toca gate cold-start; ojo AUTH-002 + DET-READY).
