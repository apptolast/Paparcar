...
class StageScaffoldTest {
    ...
    private fun replacedMethod(effect: DetectionEffect): String = when (effect) {
        ...
        is DetectionEffect.DiscardHold -> "the two discard branches of the post-confirm hold"
        is DetectionEffect.RecordHoldSettled -> "the SETTLED marker of the post-confirm hold"
    ...
    fun should_account_for_every_io_method_the_coordinator_performs_today() {
        val everyEffect = listOf(
            ...
            DetectionEffect.DiscardHold(
                com.rndeveloper.paparcar.domain.detection.HoldAction.DISCARDED_STALE, 30_000L, "steps+egress", here,
            ),
            DetectionEffect.RecordHoldSettled(30_000L, "user", here),
    ...
    fun should_default_a_handled_verdict_to_deciding_nothing_further() {
        val verdict = StageVerdict.Handled(newState = com.rndeveloper.paparcar.domain.detection.state.DetectionSessionState())
