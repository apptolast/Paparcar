...
class ArchitectureTest {
    ...
    fun `presentation layer should not import from data layer`() {
        val violations = scope
            .files
            .filter { file ->
                file.packagee?.name?.startsWith("com.rndeveloper.paparcar.presentation") == true
            }
            .flatMap { file ->
                file.imports
    ...
    fun `data layer should not import from presentation layer`() {
        val violations = scope
            .files
            .filter { file ->
                file.packagee?.name?.startsWith("com.rndeveloper.paparcar.data") == true
            }
            .flatMap { file ->
                file.imports
    ...
    fun `domain layer should not import from data or presentation layers`() {
        val violations = scope
            .files
            .filter { file ->
                file.packagee?.name?.startsWith("com.rndeveloper.paparcar.domain") == true
            }
            .flatMap { file ->
                file.imports
                    .filter { imp ->
    ...
    fun `UseCase classes should live inside the domain layer`() {
        val violations = scope
            ...
            .filter { cls ->
                cls.packagee?.name?.startsWith("com.rndeveloper.paparcar.domain") == false
            }
